<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.shangketupian" label="上课图片" prop="shangketupian">
          <file-upload
          tip="点击上传上课图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.shangketupian?ruleForm.shangketupian:''"
          @change="shangketupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.shangketupian" label="上课图片" prop="shangketupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.shangketupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
		...
		if(json.shangketupian!=''&&json.shangketupian){
                this.ruleForm.shangketupian = json.shangketupian
	    		this.ro.shangketupian = true;
		}
        } else {
	...
	this.ruleForm.rizhineirong = this.ruleForm.rizhineirong.replace(reg,'../../../springbootcd8ka/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.shangketupian!=null) {
		this.ruleForm.shangketupian = this.ruleForm.shangketupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `rizhizuoye/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.rizhizuoyeCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    shangketupianUploadChange(fileUrls) {
	this.ruleForm.shangketupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
