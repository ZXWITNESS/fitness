    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="风采内容" prop="fengcaineirong">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.fengcaineirong" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.fengcaineirong" label="风采内容" prop="fengcaineirong">
                    <span v-html="ruleForm.fengcaineirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.fengcaineirong = this.ruleForm.fengcaineirong.replace(reg,'../../../springbootcd8ka/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xueshengfengcai/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xueshengfengcaiCrossAddOrUpdateFlag = false;
						 this.parent.search();
